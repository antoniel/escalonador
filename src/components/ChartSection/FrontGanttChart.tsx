...
const FrontGanttChart: React.FC<FrontGanttChartProps> = ({ conditions, processList, schedule, play, reset }) => {
  ...
  function create_matrix() {
    ...
    for (let column = 0; column < schedule.length; column++) {
      ...
      for (let row = 0; row < processList.length; row++) {
        if (!Number.isInteger(schedule[column]) && Math.floor(schedule[column]) == row + 1) {
          ...
          last_element = schedule[column]
        } else if (schedule[column] == ChartBoxEnum.Switch && row == Math.floor(last_element) - 1) {
          newmatrix[column][row] = ChartBoxEnum.Switch
        } else if (column >= processList[row].arrivalTime && column <= last_index_list[row]) {
  ...
  function renderColumn() {
    ...
    const renderAsync = async () => {
      for (let current_time = 0; current_time < matrix.length; current_time++) {
        ...
        const column = matrix[current_time].reduce((accumulator: JSX.Element, proc, _process_index) => {
          switch (proc) {
  ...
  useEffect(() => {
    ...
    const fields = document.getElementsByClassName("process__card__fields")
  ...
  return (
    <div className="box chart">
      ...
      <p id="chart__turnaround">Turnaround: {Math.floor(turnaround * 100) / 100}</p>
      <br />
